...
public interface PlanConsumoRepository extends JpaRepository<PlanConsumo,Integer> {
    ...
    @Modifying
    @Transactional
    @Query(value = "UPDATE planes_consumo SET nombre = :nombre, tipo = :tipo, descuento_general = :descuento_genera " +//
    ...
    @Query(value = "INSERT INTO planes_consumo (id,nombre,tipo,descuento_general, id_hotel, bebidas_ilimitadas, internet_ilimitado)"+//
            " VALUES (hoteles_sequence.nextval, :nombre,:tipo,:descuento_genera, :id_hotel, :bebidas_ilimitadas, :internet_ilimitado)", nativeQuery = true)
